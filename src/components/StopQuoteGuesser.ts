...
export const StopQuoteGuesser: Button = {
    ...
    run: async (
        ...
        const rounds = await quoteGuesserSchema.find({
        ...
        const rankingEmbed = new EmbedBuilder().setTitle("Game Leaderboard");
        ...
        await quoteGuesserSchema.deleteMany({
            guildId: interaction.guildId,
            token: token,
        });
...
export const stopRound = async (
    ...
    const round = await findCurrentRound(
        quoteGuesserSchema,
        interaction.guildId!,
        token
    );
    const quoteGuesserDocument = await quoteGuesserSchema.findOne({
        guildId: interaction.guildId,
        token: token,
        round: round,
    });
    ...
    if (!quoteGuesserDocument) {
        await interaction.update({
            content: "This game doesn't exist anymore",
            embeds: [],
            components: [],
        });
        return;
    ...
    messageEmbed.setDescription(
        `"${quoteGuesserDocument.quote}" - ${quoteGuesserDocument.authorName}`
    );
    ...
    for (const wrongAnswerName of quoteGuesserDocument.wrongAnswerNames) {
        messageEmbed.addFields({
            name: wrongAnswerName,
            value: "answered wrong!",
        });
    }
